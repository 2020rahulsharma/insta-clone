...
def follow_unfollow(request):
    if request.method == 'POST':
        ...
        print(follow_user,'a new user is being is followed')
...
def archive_toggle(request, pk):
    ...
    if post.archive == False:
        post.archive = True
        post.save()
        print(request.META.get('HTTP_REFERER'),'this is http referer')
        return redirect(request.META.get('HTTP_REFERER'))
    else:
        post.archive = False
        post.save()
        return redirect(request.META.get('HTTP_REFERER'))
##request includes HTTP_REFERER field which indicates the last page the user was on######
