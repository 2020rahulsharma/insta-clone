...
class MessageCreateView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        form.instance.receiver = User.objects.get(username=self.kwargs['username'])  # ['username'] is the username assigned by to the message button in the profile.html. we are making the instance of the form assign the receiver field of the message model to the username object whos username=self.kwargs['username'] which is the storage location of url parameters
        notification = Notification(sender=self.request.user, receiver=form.instance.receiver,notification_type='message')
        notification.save()
